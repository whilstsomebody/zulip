...
class MultiuseInviteTest(ZulipTestCase):
    ...
    def test_permission_for_editing_multiuse_invite(self) -> None:
        ...
        edit multiuse invites, for all types of group values.
        """
        realm = get_realm("zulip")
        othello = self.example_user("othello")
        hamlet = self.example_user("hamlet")
        ...
        check_edit_multiuse_invite_helper(othello)
        ...
        check_edit_multiuse_invite_helper(othello, error_message="Insufficient permission")
        check_edit_multiuse_invite_helper(iago)
        ...
        check_edit_multiuse_invite_helper(desdemona, error_message="Insufficient permission")
        check_edit_multiuse_invite_helper(hamlet, error_message="Insufficient permission")
        check_edit_multiuse_invite_helper(iago)
        check_edit_multiuse_invite_helper(othello)
        ...
        check_edit_multiuse_invite_helper(othello, error_message="Insufficient permission")
        check_edit_multiuse_invite_helper(iago)
        check_edit_multiuse_invite_helper(hamlet)
    ...
    def test_permission_for_editing_invite_as(self) -> None:
        """
        Test changing invite_as based on user role hierarchy.
        """

        def check_changing_invite_as(
            created_by: UserProfile,
            edited_by: UserProfile,
            old_invite_as: int,
            new_invite_as: int,
            error_message: str | None = None,
        ) -> None:
            invite = self.create_multiuse_invite(created_by, invite_as=old_invite_as)
            result = self.api_patch(
                edited_by,
                f"/api/v1/invites/multiuse/{invite.id}",
        ...
        OWNER_INVITE = PreregistrationUser.INVITE_AS["REALM_OWNER"]
        ADMIN_INVITE = PreregistrationUser.INVITE_AS["REALM_ADMIN"]
        MODERATOR_INVITE = PreregistrationUser.INVITE_AS["MODERATOR"]
        MEMBER_INVITE = PreregistrationUser.INVITE_AS["MEMBER"]
        GUEST_INVITE = PreregistrationUser.INVITE_AS["GUEST_USER"]

        # Invite created by a member
        check_changing_invite_as(othello, othello, GUEST_INVITE, MEMBER_INVITE)
        ...
        check_changing_invite_as(othello, iago, GUEST_INVITE, MEMBER_INVITE)
        check_changing_invite_as(
            othello,
            iago,
            MEMBER_INVITE,
            MODERATOR_INVITE,
            error_message="This invitation is owned by a user without permission to give other users this role.",
        )
        check_changing_invite_as(
            othello,
            iago,
        ...
        # Invite created by a moderator
        check_changing_invite_as(shiva, shiva, GUEST_INVITE, MEMBER_INVITE)
        check_changing_invite_as(
            shiva,
            othello,
            GUEST_INVITE,
        ...
        check_changing_invite_as(shiva, iago, GUEST_INVITE, MEMBER_INVITE)
        check_changing_invite_as(
            shiva,
            iago,
            MEMBER_INVITE,
        ...
        # Invite created by an admin
        check_changing_invite_as(iago, iago, MEMBER_INVITE, ADMIN_INVITE)
        check_changing_invite_as(
            iago,
            othello,
            ADMIN_INVITE,
        ...
        check_changing_invite_as(iago, desdemona, MEMBER_INVITE, MODERATOR_INVITE)
        check_changing_invite_as(iago, desdemona, MEMBER_INVITE, ADMIN_INVITE)
        check_changing_invite_as(
            iago,
            desdemona,
            MEMBER_INVITE,
        ...
        # Invite created by an owner
        check_changing_invite_as(desdemona, desdemona, MEMBER_INVITE, OWNER_INVITE)
        check_changing_invite_as(
            desdemona,
            othello,
            GUEST_INVITE,
        ...
        check_changing_invite_as(desdemona, iago, MEMBER_INVITE, ADMIN_INVITE)
        check_changing_invite_as(
            desdemona, iago, OWNER_INVITE, ADMIN_INVITE, "Must be an organization owner"
        )

    def test_edit_multiuse_invite_original_role_permissions(self) -> None:
        """
        Test that users need appropriate permissions to edit invites based on the original role
        ...
        def check_edit_permissions(
            creator: UserProfile,
            editor: UserProfile,
            original_invite_as: int,
            error_message: str | None = None,
        ) -> None:
            invite = self.create_multiuse_invite(creator, invite_as=original_invite_as)
            result = self.api_patch(
                editor,
                f"/api/v1/invites/multiuse/{invite.id}",
            )
        ...
        # Member cannot edit invites created by other users.
        ...
        check_edit_permissions(
            admin, member, ADMIN_INVITE, error_message="Must be an organization administrator"
        )
        check_edit_permissions(
            owner, member, OWNER_INVITE, error_message="Must be an organization owner"
        )

        # Moderators have same permissions as that of members.
        check_edit_permissions(
            member, moderator, MEMBER_INVITE, error_message="Must be an organization administrator"
        )
        check_edit_permissions(
            moderator,
            moderator,
        ...
        check_edit_permissions(
            admin, moderator, ADMIN_INVITE, error_message="Must be an organization administrator"
        )
        check_edit_permissions(
            owner, moderator, OWNER_INVITE, error_message="Must be an organization owner"
        )

        # Admin can edit all users' invites except the owners..
        check_edit_permissions(member, admin, MEMBER_INVITE)
        check_edit_permissions(moderator, admin, MODERATOR_INVITE)
        check_edit_permissions(admin, admin, ADMIN_INVITE)
        check_edit_permissions(
            owner, admin, OWNER_INVITE, error_message="Must be an organization owner"
        )

        # Owner can edit all user's invites.
        check_edit_permissions(owner, owner, OWNER_INVITE)
        check_edit_permissions(admin, owner, ADMIN_INVITE)
        check_edit_permissions(moderator, owner, MODERATOR_INVITE)
        check_edit_permissions(member, owner, MEMBER_INVITE)
    ...
    def test_edit_multiuse_invite_welcome_message_custom_text(self) -> None:
        ...
        initial_welcome_message = "Welcome to our organization!"
        multiuse_invite = self.create_multiuse_invite(iago)

        # Test editing welcome message as admin.
        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{multiuse_invite.id}",
            {
                "welcome_message_custom_text": initial_welcome_message,
            },
        )
        self.assert_json_success(result)
        multiuse_invite.refresh_from_db()
        self.assertEqual(multiuse_invite.welcome_message_custom_text, initial_welcome_message)

        # Test editing welcome message to a different value
        updated_welcome_message = "Updated welcome message for new members!"
        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{multiuse_invite.id}",
            {
                "welcome_message_custom_text": updated_welcome_message,
            },
        )
        self.assert_json_success(result)
        multiuse_invite.refresh_from_db()
        self.assertEqual(multiuse_invite.welcome_message_custom_text, updated_welcome_message)
        ...
        self.assert_json_success(result)
        multiuse_invite.refresh_from_db()
        self.assertEqual(multiuse_invite.welcome_message_custom_text, "")

        # Test that non-admin cannot edit welcome message.
        result = self.api_patch(
            hamlet,
            f"/api/v1/invites/multiuse/{multiuse_invite.id}",
            {
                "welcome_message_custom_text": "Hamlet's message",
            },
        )
        self.assert_json_error(result, "Must be an organization administrator")
        multiuse_invite.refresh_from_db()
        ...
        # Test that member can edit their own invite but not add welcome message
        member_invite = self.create_multiuse_invite(hamlet)
        result = self.api_patch(
            hamlet,
            f"/api/v1/invites/multiuse/{member_invite.id}",
            {
                "welcome_message_custom_text": "Member trying to set message",
            },
        )
        self.assert_json_error(result, "Must be an organization administrator")

        # Test that admin can edit member's invite and add welcome message
        result = self.api_patch(
            iago,
            f"/api/v1/invites/multiuse/{member_invite.id}",
            {
        ...
        self.assert_json_success(result)
        member_invite.refresh_from_db()
        self.assertEqual(
            member_invite.welcome_message_custom_text, "Admin setting message for member's invite"
        )
        ...
        result = self.api_patch(
            ...
            f"/api/v1/invites/multiuse/{multiuse_invite.id}",
            {
                "welcome_message_custom_text": long_message,
            },
        )
        self.assert_json_error(
            result, "welcome_message_custom_text is too long (limit: 8000 characters)"
        )
        ...
        result = self.api_patch(
            ...
            f"/api/v1/invites/multiuse/{multiuse_invite.id}",
            {
                "welcome_message_custom_text": valid_long_message,
            },
        )
        self.assert_json_success(result)
        multiuse_invite.refresh_from_db()
        self.assertEqual(multiuse_invite.welcome_message_custom_text, valid_long_message)
